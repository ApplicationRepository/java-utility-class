...
/** 完整过程能力与性能指标 (CP/CPK/PP/PPK) 报表图片生成工具类 💡 跨平台多环境（Windows、Linux、Mac、Docker）防乱码自适应优化版 */
@Log4j2
public final class CpkPpkChartUtils {
  ...
  public static void process(
    ...
    // calculateShortTermSigma (组内标准差)：采用了一个固定系数 0.963 进行无偏估计矫正（通常对应于特定子组大小下的 $c_4$ 或 $d_2$ 倒数），用于计算
    // CP 族指标。这符合工业 SPC 的标准控制规范。
  ...
  private static JFreeChart createNormalizedChart(
      double[] data, int bins, double mean, double sigma, double lsl, double usl) {
    ...
    lineRenderer.setSeriesStroke(
        0, new BasicStroke(2.5f, BasicStroke.CAP_ROUND, BasicStroke.JOIN_ROUND));
  ...
  private static void drawMetricsPanel(
      Graphics2D g2d,
      int panelX,
      int panelY,
      int panelWidth,

...
/** 纯 Java 原生实现桑基图生成工具 💡 深度整合版：完美解决 [跨平台中文乱码] + [多层级自适应折行排版] + [图例防重叠避让] */
@Log4j2
public final class SankeyChartUtils {
   ...
   * @param height 画布总高度
   */
  public static void processSankey(
      String title, List<SankeyEntity> sankeyEntityList, String outputPath, int width, int height) {
    ...
    for (SankeyEntity entity : sankeyEntityList) {
      nodeVolumeMap.put(
          entity.getSource(),
          nodeVolumeMap.getOrDefault(entity.getSource(), 0.0) + entity.getValue());
      nodeVolumeMap.put(
          entity.getTarget(),
          nodeVolumeMap.getOrDefault(entity.getTarget(), 0.0) + entity.getValue());
    ...
    for (SankeyEntity entity : sankeyEntityList) {
      ...
      double srcTotal =
          sankeyEntityList.stream()
              .filter(e -> e.getSource().equals(src))
              .mapToDouble(SankeyEntity::getValue)
              .sum();
      ...
      if (isDoubleRow && srcLvl < levelsPerRow && tgtLvl >= levelsPerRow) {
        ...
        curve =
            new CubicCurve2D.Float(
                x1, y1, x1 + (x2 - x1) / 2f, y1, x1 + (x2 - x1) / 2f, y2, x2, y2);
      }

      // 动态注入半透明色彩，消除高维管道交错时的生硬感
      Color srcColor = PALETTE[Math.abs(src.hashCode()) % PALETTE.length];
      g2d.setColor(
          new Color(srcColor.getRed(), srcColor.getGreen(), srcColor.getBlue(), 65)); // 65 透明度层级感最佳

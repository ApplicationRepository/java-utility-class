...
/** 普通文本替换工具类 💡 深度整合版：彻底解决 [ConcurrentModificationException] 与 [XmlValueDisconnectedException] */
@Log4j2
public final class PptxTextUtils {
  ...
  private static void replaceTextInParagraph(
      XSLFTextParagraph p, String target, String replacement) {
    ...
    if (sourceFontFamily != null && !sourceFontFamily.isEmpty()) {
      String[] availableFonts =
          GraphicsEnvironment.getLocalGraphicsEnvironment().getAvailableFontFamilyNames();
      boolean fontExists =
          Arrays.stream(availableFonts).anyMatch(sourceFontFamily::equalsIgnoreCase);
      if (!fontExists) {

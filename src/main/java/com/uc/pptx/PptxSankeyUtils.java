...
public final class PptxSankeyUtils {
  ...
  public static void process(
      XMLSlideShow ppt, Map<String, SankeyData> sankeyMap, String sankeyPlaceholderMark) {
  ...
  private static void scanSankeyPlaceholders(
      XSLFShapeContainer container,
      Map<String, SankeyData> sankeyMap,
      String mark,
      Map<XSLFTextShape, SankeyData> tasks) {
    for (XSLFShape shape : container.getShapes()) {
      if (shape instanceof XSLFTextShape) {
        ...
        for (Map.Entry<String, SankeyData> entry : sankeyMap.entrySet()) {
          if (StrUtil.containsAnyIgnoreCase(fullText, entry.getKey())
              && entry.getKey().contains(mark)) {
  ...
  private static void replaceShapeWithSankey(
      XMLSlideShow ppt, XSLFSlide slide, XSLFTextShape textShape, SankeyData data) {
    ...
    byte[] sankeyImageBytes =
        generateSankeyImageBytes(data, (int) anchor.getWidth(), (int) anchor.getHeight());
  ...
  /**
   * 核心技术点：动态图表生成器 实际落地时，可以通过调用后端 ECharts 节点的 HTTP 接口， 或者使用 Java 的 Runtime 执行 PhantomJS/Node 脚本离线生成。
   */

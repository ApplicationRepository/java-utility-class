...
/** 替换表格工具类 💡 深度整合版：完美适配 [Windows/Linux/Mac/Docker] 100% 彻底干掉中文乱码与高度崩溃 */
@Log4j2
public final class PptxTableUtils {
  ...
  public static void process(
      XMLSlideShow ppt, Map<String, String[][]> tableMap, String tablePlaceholderMark) {
  ...
  private static void scanTablePlaceholders(
      XSLFShapeContainer container,
      Map<String, String[][]> tableMap,
      String tableMark,
      Map<XSLFTextShape, String[][]> tableTasks) {
      ...
      if (shape instanceof XSLFTextShape) {
        ...
        for (Map.Entry<String, String[][]> entry : tableMap.entrySet()) {
          if (StrUtil.containsAnyIgnoreCase(fullText, entry.getKey())
              && entry.getKey().contains(tableMark)) {
  ...
  private static void replaceShapeWithTable(
      XSLFSlide slide, XSLFTextShape textShape, String[][] data) {
      ...
      if (!firstPara.getTextRuns().isEmpty()) {
        ...
        if (firstRun.getFontFamily() != null && !firstRun.getFontFamily().isEmpty()) {
          ...
          String[] availableFonts =
              GraphicsEnvironment.getLocalGraphicsEnvironment().getAvailableFontFamilyNames();
          boolean fontExists =
              Arrays.stream(availableFonts).anyMatch(currentFamily::equalsIgnoreCase);
